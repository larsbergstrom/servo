...
fn default_decoder_factory() -> proc(&[u8]) -> Option<Image> {
    let foo: proc(&[u8]) -> Option<Image> = proc(data: &[u8]) { load_from_memory(data) };
    foo
}
...
mod tests {
    ...
    fn should_return_decoded_image_data_if_data_has_arrived() {
        ...
        let join_port = iamge_cache_task.wait_for_store();
    ...
    fn should_return_not_ready_if_image_is_still_decoding() {
        ...
        let decoder_factory: proc() -> proc(&[u8]) -> Option<Image> = || {
        ...
        let join_port = self.wait_for_store_prefetched();
