...
fn strip_ignorable_whitespace_from_start(opt_boxes: &mut Option<~[@RenderBox]>) {
    let mut boxes = util::replace(opt_boxes, None);
    match boxes {
        None => return,
        Some(boxes) => {
            // FIXME(pcwalton): This is slow because we can't shift more than one element at a
            // time in Rust. :(
            let mut number_to_strip = 0;
            for box in boxes.iter() {
                if box.is_whitespace_only() {
                    number_to_strip += 1
                }
            }
